...
#         return ['Error: length input strings is out of range(2, 100)']
#     surnames = []
...
#                 if len(scores) < 2:
#                     return ['Error: Number of scores must be more 2']
#                 average_score = sum(scores) / len(scores)
#                 if average_score >= 5:
#                     passed_candidates.append((surname, average_score))
#             except ValueError:
#                 return ['Error: Enter integers']
#     passed_candidates.sort(key=lambda x: (-x[1], x[0]))
#     total = [f'{surname},{average_score:.1f}' for surname, average_score in passed_candidates]
