...
# def max_sequence_sum(numbers: str, sequence_length: str) -> str:
#     if not numbers or not sequence_length:
#         return 'Error: Empty data'
#     try:
#         lst_numbers = [int(num) for num in numbers.split()]
#         length = int(sequence_length)
#     except ValueError:
#         return 'Error: Enter integers'
#     if length > len(lst_numbers):
#         return 'Error: length is out of range'
#     if length < 1:
#         return 'Error: Enter positive number'
#     positive_row = all(num > 0 for num in lst_numbers)
#     negative_row = all(num < 0 for num in lst_numbers)
#     if not positive_row and not negative_row:
#         return 'Error: Numbers must be only in positive row or in negative row'
#     increasing_sequence = []
#     decreasing_sequence = []
#     for num in range(len(lst_numbers)):
#         if not increasing_sequence or lst_numbers[num] > increasing_sequence[-1]:
#             increasing_sequence.append(lst_numbers[num])
#         if not decreasing_sequence or lst_numbers[num] < decreasing_sequence[-1]:
#             decreasing_sequence.append(lst_numbers[num])
#     if positive_row:
#         return str(sum(increasing_sequence[-length:]))
#     if negative_row:
#         return str(sum(decreasing_sequence[:length]))
#
#
# data_numbers = input()
# data_length = input()
# result = max_sequence_sum(data_numbers, data_length)
# print(result)
